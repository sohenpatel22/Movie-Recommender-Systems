...
from src.models.matrix_factorization import MatrixFactorisationCBF
...
def main():
    ...
    for seed in SEEDS:
        print("=" * 60)
        print(f"Running MF + CBF with seed {seed}")
        set_seed(seed)

        global_mean = float(train_df["rating"].mean())

        model = MatrixFactorisationCBF(
            dataset=train_set,
        ...
        model_path = MODELS_DIR / f"mf_cbf_seed_{seed}.pt"
        torch.save(train_results["model"].state_dict(), model_path)

    metrics_path = METRICS_DIR / "mf_cbf_results.pkl"
    ...
    summary_path = METRICS_DIR / "mf_cbf_summary.json"
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=4)

    print("=" * 60)
    print("Final MF + CBF results across seeds:")
